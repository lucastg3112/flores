                ...
                <div class="flex flex-col lg:flex-row gap-16">
                    <div class="lg:w-1/2 flex flex-col">
                        ...
                        <div class="glass-panel p-8 rounded-2xl mb-8 flex-grow">
                            ...
                            <p class="text-base leading-relaxed text-gray-700">{analytic_text}</p>
                        </div>
...
def get_chart_configs(indicators):
    ...
    for idx, ind in enumerate(indicators):
        ...
        # Merge existing options with responsive and maintainAspectRatio defaults
...
# Clean previous japon/arabia configs to avoid duplicates
idx = js_content.find('\n    "japon":')
if idx == -1:
    idx = js_content.find('\n    "arabia":')
if idx != -1:
...
insertion_point = js_content.find('window.renderChartsForCountry')
if insertion_point != -1:
    # Find the closing brace of chartDataConfig before window.renderChartsForCountry
    brace_idx = js_content.rfind('};', 0, insertion_point)
    if brace_idx != -1:
        new_js = js_content[:brace_idx].strip() + ',\n    "japon": ' + japon_json + ',\n    "arabia": ' + eau_json + '\n};\n\n' + js_content[insertion_point:]
        with codecs.open('e:\\pagina web aldea global 2\\assets\\js\\charts-entrega3.js', 'w', 'utf-8') as f:
            f.write(new_js)
        print("JS updated.")
    else:
        print("Could not find brace before window.renderChartsForCountry in JS.")
else:
    print("Could not find window.renderChartsForCountry in JS.")
